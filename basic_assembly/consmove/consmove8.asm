; lwasm consmove8.asm -fbasic -oconsmove8.bas --map --list
; a09 -frsdos -Bconsmove8-exec.bas -oconsmove8-exec.bin consmove8-exec.asm
; a09 -fbasic -L63001 -N1 -oconsmove8-exec.bas consmove8-exec.asm
; decb copy -2 consmove8.bin drive0.dsk,CONSMOVE.BIN

; Allow embedded characters to move the cursor in a PRINT

; v8 - correct start code for position indepenent-ness
;    - remove text output ("ON" and "OFF")
;    - change default to be for 32K system (&H7E00 load address)

; Load at $3e00 (16K) or $7e00 (32K).
;ORGADDR     equ     $3e00       ; Where program loads in memory
ORGADDR     equ     $7e00       ; Where program loads in memory
...
start       leay    start,pcr   ; Y=start
            ...
            ldx     <EXECJP     ; else, load Y with EXECJP address
            leay    start,pcr   ; Y=start ... again
            pshs    y           ; push Y onto the stack
            cmpx    ,s++        ; compare X to Y value on stack and pop
            beq     toggle      ; if yes, goto toggle
                                ; else, must be USR
...
defaults    lda     #UPCHAR
            ...
            lda     #RIGHTCHAR
            sta     right,pcr
            bra     exitsuccess ; TODO: reorganize to use just "bra"
...
cantmove    PULS A,B,X,PC           ; what the ROM does.
